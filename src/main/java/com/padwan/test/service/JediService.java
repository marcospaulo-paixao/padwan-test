...
public class JediService {
    ...
    public JediDTO incluir(JediDTO jediDTO) {
        ...
        if (jediDTO.getIdMentor() != null && jediDTO.getIdMentor() != 0) {
            ...
            RelacionamentoJediDTO relacionamentoJediDTO = new RelacionamentoJediDTO(
                    relacionamentoJediRepository.save(relacionamentoJedi));
    ...
    public MestreEAprendizDTO processarMestreEAprendiz(List<Jedi> jedis, List<RelacionamentoJedi> relacionamentoJedis,
            Jedi mentor) {
    ...
    public Connection conectar(Connection c) {
        try {
            ...
            c = DriverManager
                    .getConnection("jdbc:postgresql://192.168.0.103:15432/padawan",
    ...
    public Map consultaQuantidadeMidichlorians(Integer id) {
        Optional<Jedi> opt = this.repository.findById(id);
        if (opt.isEmpty()) {
            throw new RuntimeException("Jedi não encontrado!");
        }
        Jedi jedi = opt.get();
        Map<String, Object> json = new HashMap<>();
        json.put("midichlorians",jedi.getMidichlorians());
        return json;
    }

    public AtualizaJediDTO atualizaJedi(AtualizaJediDTO jediUpdateDTO) {
        validaJedi(jediUpdateDTO);
        Jedi aprendiz = new Jedi();
        Jedi mentor = this.repository.findById(jediUpdateDTO.getIdMentor()).get();
        RelacionamentoJedi relacionamentoJedi = null;

        aprendiz.setId(jediUpdateDTO.getIdAprendiz());
        aprendiz.setNome(jediUpdateDTO.getNome());
        aprendiz.setStatusEnum(jediUpdateDTO.getStatus());

        this.repository.save(aprendiz);
        relacionamentoJedi = this.relacionamentoJediRepository.findByIdMentorAndIdAprendiz(mentor.getId(),
                aprendiz.getId());

        if (relacionamentoJedi == null) {
            relacionamentoJedi = new RelacionamentoJedi();
            relacionamentoJedi.setIdAprendiz(aprendiz.getId());
            relacionamentoJedi.setIdMentor(mentor.getId());
            this.relacionamentoJediRepository.save(relacionamentoJedi);
        }

        return new AtualizaJediDTO(aprendiz, mentor);
    }

    public void validaJedi(AtualizaJediDTO jediUpdateDTO) {
        if (jediUpdateDTO.getIdAprendiz() == null || jediUpdateDTO.getIdAprendiz() <= 0) {
            throw new RuntimeException("O id do aprendiz é inválido!");
        }
        if (jediUpdateDTO.getIdMentor() == null || jediUpdateDTO.getIdMentor() <= 0) {
            throw new RuntimeException("O id do aprendiz é inválido!");
        }
        if (this.repository.findById(jediUpdateDTO.getIdAprendiz()).isEmpty()) {
            throw new RuntimeException("Aprendiz não encontrado!");
        }
        if (this.repository.findById(jediUpdateDTO.getIdMentor()).isEmpty()) {
